...
export function validateQueryParams(query: QueryParams) {
  ...
  const validPageNumber = (isNaN(pageNumber) || pageNumber <= 0) ? defaultPage : pageNumber;

  // Validate and sanitize `limit`
  const pageSize = parseInt(query.limit as string, 10);
  const validPageSize = (isNaN(pageSize) || pageSize <= 0) ? defaultLimit : pageSize;
  ...
  const sortDirection = validSortOrders.includes(query.sortOrder as string) ? (query.sortOrder as 'asc' | 'desc') : defaultSortOrder;

  // Validate and sanitize `minPrice` and `maxPrice`
  const minPrice = parseFloat(query.minPrice as string);
  const validMinPrice = (isNaN(minPrice) || minPrice < 0) ? defaultMinPrice : minPrice;

  const maxPrice = parseFloat(query.maxPrice as string);
  const validMaxPrice = (isNaN(maxPrice) || maxPrice < validMinPrice) ? defaultMaxPrice : maxPrice;

  // Validate and sanitize `distance`
  const distance = parseFloat(query.distance as string);
  const validDistance = (isNaN(distance) || distance < 0) ? defaultDistance : distance;

  // Validate and sanitize `lat` and `lng`
  const lat = parseFloat(query.lat as string);
  const validLat = (isNaN(lat) || lat < -90 || lat > 90) ? defaultLat : lat;

  const lng = parseFloat(query.lng as string);
  const validLng = (isNaN(lng) || lng < -180 || lng > 180) ? defaultLng : lng;
